...
def send_to_rabbitmq(movie_id, file_url):
    ...
    try:
        ...
        connection.close()
        logger.info(f"Сообщение для фильма {movie_id} успешно отправлено в RabbitMQ: {file_url}")
...
class UploadFileView(APIView):
    ...
    def post(self, request, movie_id, content_type):
        logger.info(f"Получен запрос на загрузку файла для movie_id={movie_id}, content_type={content_type}")
        ...
        try:
            with FTP() as ftp:
                logger.debug(f"Подключение к FTP {settings.FTP_SERVER_HOST}:{settings.FTP_SERVER_PORT}...")
            ...
            logger.info(f"Загрузка завершена успешно для movie_id={movie_id}, content_type={content_type}")
            return JsonResponse({
                'movie_id': movie_id,
            ...
            logger.error(f"Ошибка при загрузке файла для movie_id={movie_id}, content_type={content_type}: {e}", exc_info=True)
    ...
    def _ensure_ftp_dir(self, ftp, path):
        ...
        for dir in directories:
            ...
            try:
                ...
                logger.error(f"Неожиданная ошибка при работе с директорией {current}: {e}")
                raise

def _delete_original_video_files(ftp_host: str, ftp_user: str, ftp_pass: str, base_path: str, movie_id: int):
    ...
    with ftputil.FTPHost(ftp_host, ftp_user, ftp_pass) as ftp:
        for fname in original_video_names:
            ...
            try:
                ...
                logger.error(f"Ошибка при удалении оригинального видеофайла {original_video_path}: {e}")
                
    return deleted_any

def _delete_transcoded_files(startswith: str, ftp_host: str, ftp_user: str, ftp_pass: str, base_path: str, movie_id: int):
    ...
    with ftputil.FTPHost(ftp_host, ftp_user, ftp_pass) as ftp:
        ...
        try:
            ...
            logger.error(f"Ошибка при получении списка файлов из {transcoded_dir}: {list_err}")
            return 0
        ...
        for item in items:
            ...
            try:
                ...
                logger.warning(f"Не удалось удалить транскодированный файл {item_path}: {e}")
                
        logger.info(f"Удалено {deleted_count} транскодированных файлов из {transcoded_dir}")
...
def _remove_empty_dir(ftp, transcoded_dir: str):
    ...
    try:
        ...
        if not remaining_items:
            logger.info(f"Папка транскодированных файлов {transcoded_dir} пуста, попытка удаления папки.")
            ftp.rmdir(transcoded_dir)
            logger.info(f"Удалена пустая папка транскодированных файлов: {transcoded_dir}")
            return True 
        else:
            logger.info(f"Папка транскодированных файлов {transcoded_dir} не пуста, оставлено {len(remaining_items)} элементов.")
            return False
    except Exception as e:
        logger.warning(f"Ошибка при проверке/удалении папки {transcoded_dir}: {e}")
        return False

def _delete_simple_file(ftp_host: str, ftp_user: str, ftp_pass: str, base_path: str, file_type: str):
...
def delete_movie_files_from_ftp(movie_id: int, file_type: str, ftp_host: str, ftp_user: str, ftp_pass: str) -> bool:
    """
    ...
    logger.info(f"Начало удаления файлов для movie_id={movie_id}, тип={file_type}, базовый путь={base_path}")
    ...
    try:
        if file_type == 'm':
            ...
            delete_status = _delete_original_video_files(ftp_host, ftp_user, ftp_pass, base_path, movie_id)
            
            
            delete_status = _delete_transcoded_files("m", ftp_host, ftp_user, ftp_pass, base_path, movie_id)
            
        elif file_type in ['p', 't']:
            
            delete_status = _delete_simple_file(ftp_host, ftp_user, ftp_pass, base_path, file_type)
        ...
        if delete_status:
            logger.info(f"Завершено удаление файлов для movie_id={movie_id}, тип={file_type}")
        return delete_status
        
    except Exception as e:
        logger.error(f"Ошибка при удалении файлов фильма {movie_id} типа {file_type} с FTP: {e}")
        return False
...
class DeleteFileView(APIView):
    ...
    def delete(self, request, movie_id, content_type):
        ...
        if not all([ftp_host, ftp_user, ftp_pass]):
            logging.error("Не заданы параметры подключения к FTP (FTP_SERVER_HOST, FTP_SERVER_USER, FTP_SERVER_PASSWORD)")
            return JsonResponse(
        ...
        try:
            success = delete_movie_files_from_ftp(movie_id, internal_file_type, ftp_host, ftp_user, ftp_pass)

            if success:
                logging.info(f"Файлы типа '{internal_file_type}' для фильма ID {movie_id} успешно удалены с FTP.")
                return JsonResponse(
                    {'message': f'Файлы типа \'{content_type}\' для фильма успешно удалены.'},
                    status=status.HTTP_200_OK
                )
            else:
                logging.warning(f"Не удалось удалить файлы типа '{internal_file_type}' для фильма ID {movie_id}.")
                return JsonResponse(
            ...
            logging.error(f"Ошибка в DeleteFileView при удалении movie_id={movie_id}, content_type={content_type}: {e}", exc_info=True)
            return JsonResponse(
                {'error': 'Внутренняя ошибка сервера при удалении файла'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

def _remove_movie_folder(ftp_host: str, ftp_user: str, ftp_pass: str, base_path: str, movie_id: int) -> bool:
    """
    ...
    try:
        with ftputil.FTPHost(ftp_host, ftp_user, ftp_pass) as ftp:
            ...
            if not ftp.path.isdir(base_path):
                logger.warning(f"Путь {base_path} существует, но не является директорией.")
                return False 
            ...
            try:
                ...
                if not items:
                    ...
                    logger.warning(f"Папка фильма {base_path} не пуста. Удаление невозможно. Оставшиеся элементы: {items}")
                    
            except Exception as list_err:
                logger.error(f"Ошибка при проверке содержимого папки фильма {base_path}: {list_err}")
...
class DeleteAllMovieFilesView(APIView):
    ...
    def delete(self, request, movie_id):
        ...
        base_path = f"{settings.FTP_STORAGE_LOCATION}/movies/{movie_id}"
        logger.info(f"Начало удаления всех файлов и папки для movie_id={movie_id}, базовый путь={base_path}")
        ...
        if files_deleted_successfully: 
            folder_deleted = _remove_movie_folder(ftp_host, ftp_user, ftp_pass, base_path, movie_id)
        else:
            logger.warning(f"Пропуск удаления папки фильма {base_path} из-за ошибок при удалении файлов.")
        
                
        all_files_results = list(deletion_results.values())
        all_files_handled = all_files_results and all(r is not False for r in all_files_results) 
