...
USER_SERVICE_PRIVILEGES_URL = getattr(settings, 'USER_SERVICE_PRIVILEGES_URL', 'http://user-service:8000/privileges/')
...
class ExternalJWTAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if not user_id_from_token or not username_from_token:
            raise AuthenticationFailed('Token payload is missing required user information (user_id, username).')
        ...
        if 'is_staff' not in privileges_data or 'is_superuser' not in privileges_data:
             raise AuthenticationFailed('User service response missing is_staff/is_superuser fields.')
